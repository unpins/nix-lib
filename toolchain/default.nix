...
{ origPkgs, unpinPackTool }:
        let
          ...
          # argv[0] faces `unpin` materializes as symlinks at install. Every name
          # here must exist in the driver's multicall table or the symlink installs
          # broken — `embedVfs` asserts that against the table the compile actually
          # generated, because this list is hand-written and the table is not.
          driverFaces = [
            "clang" "clang++" "cc" "c++"
            "ld.lld"
          ...
          embedVfs = core: pkgs.stdenv.mkDerivation {
            ...
            buildCommand = ''
              ...
              # Aliases — newline-separated, no trailing newline (matches
              # withUnpinEmbed's writer).
              mkdir -p "$__stage/unpin"
              printf '%s' '${origPkgs.lib.concatStringsSep "\n" driverFaces}' \
                > "$__stage/unpin/aliases"

              # Announced must equal dispatched. `cc`/`c++` shipped here for months
              # while the driver had no table entry for them, so `cc --version`
              # printed the subcommand list and exited 1 — nothing compared the two
              # lists. The core's table is generated by CMake from the tool/alias
              # registrations, with the `llvm-` prefix stripped, so strip it here
              # too before looking a face up. Static: no execution, so this holds on
              # every build host, cross included.
              __undispatched=""
              for __f in ${origPkgs.lib.escapeShellArgs driverFaces}; do
                grep -qxF "''${__f#llvm-}" "${core}/lib/llvm-driver-tools" \
                  || __undispatched="$__undispatched $__f"
              done
              if [ -n "$__undispatched" ]; then
                echo "embedVfs: unpin/aliases announces faces the driver does not dispatch:$__undispatched" >&2
                echo "embedVfs: dispatch table has:" >&2
                sed 's/^/  /' "${core}/lib/llvm-driver-tools" >&2
                exit 1
              fi
...
static cl::SubCommand LinkSub(LinkSubName, "Link LLVM bitcode/IR modules");' \
  ...
  cl::ParseCommandLineOptions(static_cast<int>(LinkArgv.size()),
          ...
          installPhase = ''
            ...
            # The names this binary actually dispatches, straight from the table
            # CMake generated. Build metadata for `embedVfs`'s announced-must-be-
            # dispatched assert; not carried into the payload. Plain names, so it
            # adds no store reference.
            sed -n 's/^LLVM_DRIVER_TOOL("\([^"]*\)".*/\1/p' \
              tools/llvm-driver/LLVMDriverTools.def | sort -u > "$out/lib/llvm-driver-tools"
            test -s "$out/lib/llvm-driver-tools"
            runHook postInstall
          '';
