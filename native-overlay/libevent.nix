...
# `__atomic_fetch_add_8`). Pass `pkgs.pkg-config` (splicing-aware) so the
# cross-correct `<triple>-pkg-config-wrapper` is picked.
{ lib }:
pkgs:
pkgs.libevent.overrideAttrs (oa: {
  nativeBuildInputs = (oa.nativeBuildInputs or [ ]) ++ [ pkgs.pkg-config ];
})
